...
function Suggest(props) {
  const { items, history, musicAction: { startSong } } = props

  const handleItemClick = (item, childItem) => {
    if (item.type === 'song') onClickSong(childItem)
    if (item.type === 'mv') onClickMv(childItem)
    if (item.type === 'playlist') onClickPlaylist(childItem)
  }
  ...
  const onClickSong = item => {
    const { id, name, artists, duration, mvid, album: { id: albumId, name: albumName } } = item
    const song = createSong({ id, name, artists, duration, albumId, albumName, mvId: mvid })
    startSong(song)
  }
        ...
        items.map((item, index) => {
          return (
            <div className='search-result-wrapper' key={index}>
              ...
              <ul>
                {
                  item.data && item.data.length && item.data.map((childItem, childIndex) => {
                    return (
                      <li
